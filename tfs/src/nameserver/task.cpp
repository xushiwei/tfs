...
namespace tfs
{
  namespace nameserver
  {
    ...
    void LayoutManager::Task::dump(int32_t level, const char* const format)
    {
      ...
      {
        TBSYS_LOGGER.logMessage(level, __FILE__, __LINE__, __FUNCTION__, pthread_self(), "pointer %p, %s plan seqno: %"PRI64_PREFIX"d, type: %s ,status: %s, priority: %s , block_id: %u, begin: %"PRI64_PREFIX"d, end: %"PRI64_PREFIX"d, runer: %s",
            this,
            format == NULL ? "" : format,
            seqno_,
    ...
    void LayoutManager::CompactTask::dump(int32_t level, const char* const format)
    {
      ...
      if (level <= TBSYS_LOGGER._level)
      {
        TBSYS_LOGGER.logMessage(level, __FILE__, __LINE__, __FUNCTION__, pthread_self(), "pointer: %p, %s plan seqno: %"PRI64_PREFIX"d, type: %s ,status: %s, priority: %s , block_id: %u, begin: %"PRI64_PREFIX"d, end: %"PRI64_PREFIX"d, runer: %s, complete status: %s",
            this,
            format == NULL ? "" : format,
            seqno_,
            type_ == PLAN_TYPE_REPLICATE ? "replicate" : type_ == PLAN_TYPE_MOVE ? "move" : type_ == PLAN_TYPE_COMPACT
